 ...
 * @since 1.6
 */
class BP_Gifts_Component extends BP_Component {
	...
	function setup_globals() {
		...
		$global_tables = array(
			'table_name'      => $bp->table_prefix . 'bp_gifts'
		);
		$bp->gifts->id = 'gifts';
		...
		$globals = array(
			'slug'                  => BP_GIFTS_SLUG,
			'root_slug'             => isset( $bp->pages->{$this->id}->slug ) ? $bp->pages->{$this->id}->slug : BP_GIFTS_SLUG,
			'has_directory'         => true, // Set to false if not required
			'notification_callback' => 'bp_gifts_format_notifications',
	 ...
	 * BP_Gifts_Component::setup_admin_bar().
	 *
	 * @global obj $bp
	 */
	/*
	function setup_nav() {
		// Add 'Gifts' to the main navigation
		$main_nav = array(
			'name' 		      => __( 'Gifts', 'bp-gifts' ),
			'slug' 		      => bp_get_gifts_slug(),
			'position' 	      => 80,
			'screen_function'     => 'bp_gifts_screen_one',
			'default_subnav_slug' => 'screen-one'
		);

		$example_link = trailingslashit( bp_loggedin_user_domain() . bp_get_gifts_slug() );

		// Add a few subnav items under the main Gifts tab
		$sub_nav[] = array(
			'name'            =>  __( 'Screen One', 'bp-gifts' ),
			'slug'            => 'screen-one',
			'parent_url'      => $gifts_link,
			'parent_slug'     => bp_get_gifts_slug(),
			'screen_function' => 'bp_gifts_screen_one',
			'position'        => 10
		);
		...
		$sub_nav[] = array(
			'name'            =>  __( 'Screen Two', 'bp-gifts' ),
			'slug'            => 'screen-two',
			'parent_url'      => $gifts_link,
			'parent_slug'     => bp_get_gifts_slug(),
			'screen_function' => 'bp_gifts_screen_two',
			'position'        => 20
		);
		...
		bp_core_new_subnav_item( array(
			...
			'screen_function' => 'bp_gifts_screen_settings_menu',
			'position' 	  => 40,
	...
	*/
	 ...
	 * hook to init. See BP_Gifts_Component::__construct().
	 *
	 * @package BuddyPress_Skeleton_Component
	 * @since 1.6
	...
	function register_post_types() {
		...
		$labels = array(
			'name'	   => __( 'High Fives', 'bp-gifts' ),
			'singular' => __( 'High Five', 'bp-gifts' )
		);

		// Set up the argument array for register_post_type()
		$args = array(
			'label'	   => __( 'High Fives', 'bp-gifts' ),
			'labels'   => $labels,
			'public'   => false,
			'show_ui'  => true,
 ...
 * BP_Gifts_Component is loaded after BuddyPress's core components. This is a good thing because
 ...
 * @since 1.6
 */
function bp_gifts_load_core_component() {
	global $bp;

	$bp->gifts = new BP_Gifts_Component;
}
